...
module.exports = Utils =
  ...
  mapFiles: (resolveRoot, files, stripPrefixes) ->
    ...
    files = files.map (f) -> 
      path.resolve resolveRoot.toString(), f.toString()
    # And that the strip prefixes all end with a /, to avoid a target path being absolute.
    stripPrefixes = stripPrefixes.map (p) -> path.join( "#{path.resolve resolveRoot, p.toString()}#{CompatibilityHelpers.pathSep}" )
